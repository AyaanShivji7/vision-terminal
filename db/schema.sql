-- Vision Terminal — baseline Postgres schema
--
-- This file is the source of truth for the database. Re-running it is
-- idempotent (every CREATE uses IF NOT EXISTS) so a fresh clone can run it
-- once against an empty Neon database and be ready to go.
--
-- For incremental changes, add a numbered migration in db/migrations/ and
-- also reflect the change here so schema.sql stays authoritative.
--
-- Usage (local):
...
-- ---------------------------------------------------------------------------
-- Portfolio holdings (manual entries, scoped per Clerk user)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Daily top picks (AI-generated, shared across all users)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- App users table (optional companion to Clerk).
--
-- Clerk is the source of truth for identity and publicMetadata.role. This
-- table is an app-side cache so that cron jobs and background workers that
-- don't have a Clerk session can still resolve roles + audit history.
-- ---------------------------------------------------------------------------
